...
import { ShopifyMarketplaceOptions } from "./shopify-marketplace-options"
import type { JudgmentResult } from "./ai-judgment"
import { useRouter } from "next/navigation"

interface ShopifyReturnItemsProps {
  orderNumber?: string
...
export function ShopifyReturnItems({ orderNumber, shippingAddress, analysisResult }: ShopifyReturnItemsProps) {
        ...
        <div key={orderId} className="bg-white border border-gray-200 rounded-lg shadow-sm">
          <div className="p-4 border-b border-gray-200 bg-[#f3f3f3]">
            <div className="flex items-center text-sm text-gray-600">
              <CheckCircle2 className="h-5 w-5 text-[#008060] mr-2" />
              <div>
                ...
                <div className="flex items-center">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <h3 className="font-medium text-[#008060] hover:text-[#006e52] hover:underline cursor-pointer">
                        {item.name}
                      </h3>
                      <button
                        onClick={() => toggleItemDescription(item.id)}
                        className="text-xs text-[#008060] hover:text-[#006e52] hover:underline flex items-center"
                      >
                        Item details
