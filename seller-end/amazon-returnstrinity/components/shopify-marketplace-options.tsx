...
interface ShopifyMarketplaceOptionsProps {
  itemName: string
  condition: string
...
export function ShopifyMarketplaceOptions({
  itemName,
  condition,
  originalPrice,
  analysisResult,
  uploadedImages,
}: ShopifyMarketplaceOptionsProps) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
      <h3 className="text-lg font-medium mb-4 text-[#004C3F]">Marketplace Options</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        {options.map((option) => (
          <div
            key={option.name}
            className={`border ${selectedOption === option.name ? "border-[#008060]" : "border-gray-200"} rounded-md overflow-hidden cursor-pointer`}
              ...
              {option.name === "Facebook Marketplace" ? (
                <button
                  onClick={() => {
                    ...
                    router.push("/shopify/facebook-demo")
                  }}
