...
export function SocialCreditGauge({ score = 75 }: SocialCreditGaugeProps) {
  ...
  useEffect(() => {
    ...
    // Create a linear gradient for the gauge
    ...
    // Fixed needle position at 2:30 (75 on a 0-100 scale)
    const fixedScore = 75
    const needleLength = radius - 10
    const needleAngle = Math.PI - (fixedScore / 100) * Math.PI
  ...
  const getScoreColor = () => {
    return "text-cyan-500" // Color for score 75
  }
